...
public class Road {
	...
	public Collection<Road> intersections;
	Direction direction;
	...
	public void assignGate(CrossingGate gate){
		gates.add(gate);
		if (carFactory != null)
			carFactory = new CarFactory(direction, new Point(startX-roadSize/2,startY), gates, null, true);  // allows additional gates.  Needs fixing
	}

	//give factorys a reference to the road and allow "dead" factories
	public void addCarFactory(Road road, boolean spawnCars){
		if (carFactory == null) // We only allow one
			carFactory = new CarFactory(direction, new Point(startX-roadSize/2,startY), gates, road, spawnCars);
	}
	...
	public void addIntersection(Road road) {
